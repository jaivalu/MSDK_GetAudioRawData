...
/// </summary>
void LoadConfig() {
	const std::string API_KEY = "Zw5VVjg8S_2Nzt_mK1kDVg";  // Zoom Meeting SDK Key or Client ID
	const std::string API_SECRET = "Bcwu52lbzCxYYsiPjd78Ol8enwrvgHNo";  // Zoom Meeting SDK Secret or Client Secret
	const std::string MEETING_NUMBER = "85112856749";  // Meeting number as a string
	const UINT64 m_no = 85112856749;
	const string PASSCODE = "85112856749";  // Meeting number as a string

	const int ROLE = 0;  // 0 for participant, 1 for host
	auto current_time_point = std::chrono::system_clock::now();
	auto expiration_time_point = current_time_point + std::chrono::hours(2);  // Token valid for 2 hours

	// Convert to time_t for iat and exp fields
	//std::time_t current_time = std::chrono::system_clock::to_time_t(current_time_point);
	//std::time_t expiration_time = std::chrono::system_clock::to_time_t(expiration_time_point);

	// Create the payload and generate the JWT

	auto token = jwt::create()
	.set_payload_claim("appKey", jwt::claim(API_KEY))
	.set_payload_claim("sdkKey", jwt::claim(API_KEY))
		.set_payload_claim("mn", jwt::claim(MEETING_NUMBER))
		.set_payload_claim("role", jwt::claim(std::to_string(ROLE)))
		.set_payload_claim("iat", jwt::claim(current_time_point))
		.set_payload_claim("exp", jwt::claim(expiration_time_point))
		.set_payload_claim("tokenExp", jwt::claim(expiration_time_point))
		.sign(jwt::algorithm::hs256{ API_SECRET });  // Signing with HS256 and the secret key

	std::cout << "JWT Token: " << token << std::endl;
	
	sdk_jwt = StringToWString(token);
	meeting_number = m_no;
	passcode = StringToWString(PASSCODE);
...
int main()
{
	...
	auto recognizer = sharedResource.GetRecognizer();
